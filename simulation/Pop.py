...
class Pop:
    ...
    def infected(self, infection_day: int):
        ratio_healthy = (c.total_pop - Pop.total_infected) / c.total_pop
        Pop.total_infected += 1

        self.state = 1
        self.day_infected = infection_day
        self.outcome_death = np.random.binomial(1, c.case_fatality_rate)
        ...
        amount_infected_theory = np.random.poisson(c.R)
        amount_infected_practice = sum([1 for rand in np.random.random(amount_infected_theory) if rand > ratio_healthy])
        infection_times = np.random.normal(c.mean_infection_time, c.sigma_infection_time, amount_infected_practice)
